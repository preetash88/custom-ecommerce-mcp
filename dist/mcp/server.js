...
mcp_server.registerTool("add_product", {
    title: "Add Product",
    description: "Create a new product with sku, name, description(optional but better to have), price, and quantity.",
...
mcp_server.registerTool("get_product_by_id", {
...
mcp_server.registerTool("delete_product", {
    title: "Delete a Product by ID",
    description: "Delete a product using its unique ID(number). Returns true if deletion was successful, false otherwise.",
    inputSchema: {
        id: z.number().int().positive(),
    },
...
mcp_server.registerTool("update_product", {
    title: "Update Product",
    description: "Update an existing product by providing correct id(mandatory) as input along with with sku(optional), name(optional), description(optional but better to have), price(optional), and quantity(optional).",
...
mcp_server.registerResource("product_catalog", "products://catalog", {
    title: "Product Catalog",
    description: "A catalog of products available in the e-commerce store, including their details such as SKU, name, description, price, and quantity in stock.",
}, async (uri) => {
...
mcp_server.registerResource("low_product_inventory", "products://low_inventory", {
    title: "Low Product Inventory",
    description: "A list of products that have low inventory levels and may need restocking soon.",
}, async (uri) => {
    const products = await ps.listProducts(100, 0);
...
mcp_server.registerTool("add_product_smartly", {
    title: "Add Product Smartly",
    description: "Create a new product with sku, name, description(optional), price, and quantity. Since description is optional, is description is missing, use smart AI generated description if missing. This tool also checks for existing products with the same SKU to prevent duplicates.",
    inputSchema: inputProductSchema.shape,
    outputSchema: outputProductSchema.shape,
}, async (raw) => {
    ...
    if (!finalDescription) {
        ...
        finalDescription =
            response.content.text || "No description available.";
    }
    const output = await ps.addProduct({
        ...args,
...
mcp_server.registerPrompt("generate_product_description_template", {
    title: "Generate Product Description Template",
    description: "Generates a concise and appealing product description template based on the product name provided.",
    argsSchema: {
